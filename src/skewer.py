...
from kebab_spot import KebabSpot, spot_create, spot_size, spot_name, \
    spot_has, spot_string_em, total_calories, is_vegan
...
def skewer_calories(skewer):
    """
    Returns the total amount of calories
    :param skewer:
    :return:
    """
    if skewer.top is None:
        ...
        return total_calories(skewer.top)

def skewer_vegan(skewer):
    """
    returns whether is_vegan is True or False
    :param skewer:
    :return:
    """
    if is_vegan(skewer.top) is True:
        return True
    else:
        return False